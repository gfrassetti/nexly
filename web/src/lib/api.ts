...
export async function fetchJson<T>(path: string, opts: RequestInit = {}, token?: string) {
  const res = await fetch(join(path), { ...opts, headers: { ...authHeaders(token), ...(opts.headers || {}) } });
  return handle<T>(res);
}

export const getContacts = (token: string) => fetchJson<any[]>("/contacts", { method: "GET" }, token);

export const getMessages = (token: string, params: { contactId?: string; provider?: string } = {}) => {
...
export const createContact = (token: string, body: { name: string; phone: string; email?: string; tags?: string[] }) =>
